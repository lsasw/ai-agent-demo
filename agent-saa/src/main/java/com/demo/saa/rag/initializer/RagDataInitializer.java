...
public class RagDataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        if (!ragConfig.isPipelineReady()) {
            log.warn("DashScope Pipeline not ready, skipping seed. Create at https://dashscope.console.aliyun.com/rag");
            return;
        }
        if (!ragService.listDocuments().isEmpty()) {
            log.info("KB has docs, skip seeding");
            return;
        }
        ...
        seed("Claude Code Installation", "install",
            ...
            + "CLI: claude, claude -p, claude -c, claude update. Session: /clear, /help, /exit, /model.");
        ...
        seed("Claude Code How It Works", "concept",
            ...
            + "2) Take Action via 5 tool categories: File Ops, Search, Execute, Network, Code Intelligence. "
            + "3) Verify Results: checks each step, auto-retries on failure. "
            + "Models: Sonnet (most coding), Opus (complex reasoning), Haiku (fast), Fable 5 (highest tier). "
        ...
        seed("Claude Code CLI Reference", "reference",
            ...
            + "claude -r name (resume), claude update, claude install [ver], claude agents (monitor). "
            + "Pipes: cat logs | claude -p 'analyze'. "
        ...
        seed("Claude Code Common Workflows", "workflow",
            ...
            + "6) CI/CD: claude -p 'check security vulnerabilities' for GitHub Actions/GitLab CI. "
